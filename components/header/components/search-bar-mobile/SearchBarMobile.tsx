...
import SearchOptionMobile from '@/components/search-option-mobile';
import {
  ChevronLeftIcon,
  ChevronRightIcon,
  MagnifyingGlassIcon,
...
const SearchBarMobile = ({ exploreNearby, searchPage }: SearchBarMobileProps) => {
      ...
      >
        ...
        <div className="mt-6">
          ...
          {exploreNearby.slice(0, 4).map((data, index) => (
            <Nearby key={index} data={data} isSmall />
          ))}
        </div>
      </section>
      <SearchOptionMobile
